                        ...
                        <div><label class="block text-sm font-medium text-gray-700">Case Manager</label>
                            <select name="case_manager_id" class="mt-1 block w-full rounded-md border border-gray-300 px-3 py-2 shadow-sm focus:border-[#0092b4] focus:outline-none focus:ring-1 focus:ring-[#0092b4] text-sm">
                                ...
                                @foreach($caseManagers as $cm)
                                    <option value="{{ $cm->id }}" @selected(($enquiry->case_manager_id ?? $enquiry->selectedCaseManager?->id) == $cm->id)>{{ $cm->first_name }} {{ $cm->last_name }}</option>
                                @endforeach
                            </select>
                        </div>
        ...
        <div class="space-y-6">
            ...
            <div class="rounded-lg border border-gray-200 bg-white p-6">
                ...
                <div class="space-y-4">
                    ...
                    <div class="border-b border-gray-100 pb-4 last:border-0 last:pb-0" x-data="{ editing: false }">
                            ...
                            <div class="min-w-0 flex-1">
                                <div class="flex items-center justify-between gap-2">
                                    <div class="flex items-center gap-2 text-sm">
                                        ...
                                        <span class="text-xs text-gray-400">{{ $comm->communication_date ? \Carbon\Carbon::parse($comm->communication_date)->format('d/m/Y H:i') : $comm->created_at->format('d/m/Y') }}</span>
                                    </div>
                                    <div class="flex items-center gap-2 shrink-0">
                                        <button type="button" @click="editing = !editing" class="text-xs text-[#0092b4] hover:underline"><i class="fa-solid fa-pen mr-1"></i>Edit</button>
                                        <form method="POST" action="{{ route('communications.destroy', $comm) }}" data-swal="Delete this communication?">
                                            @csrf @method('DELETE')
                                            <button type="submit" class="text-xs text-red-500 hover:underline"><i class="fa-solid fa-trash mr-1"></i>Delete</button>
                                        </form>
                                    </div>
                                </div>
                                <p class="text-sm font-medium text-gray-700">{{ $comm->subject }}</p>
                                <p class="text-sm text-gray-500" style="white-space:pre-wrap;">{{ $comm->summary }}</p>
                        ...
                        <div x-show="editing" x-cloak class="mt-3 ml-11">
                            <form method="POST" action="{{ route('communications.update', $comm) }}" class="grid gap-3 rounded-md border border-gray-200 bg-gray-50 p-4 sm:grid-cols-2">
                                @csrf @method('PATCH')
                                <div>
                                    <label class="block text-xs font-medium text-gray-500">Type</label>
                                    <select name="type" class="mt-1 block w-full rounded-md border border-gray-300 px-3 py-2 text-sm shadow-sm focus:border-[#0092b4] focus:outline-none focus:ring-1 focus:ring-[#0092b4]">
                                        @foreach(['Email','Phone','Letter','Meeting','WhatsApp','LinkedIn','Follow Up','Other'] as $t)
                                        <option value="{{ $t }}" @selected($comm->type === $t)>{{ $t }}</option>
                                        @endforeach
                                    </select>
                                </div>
                                <div>
                                    <label class="block text-xs font-medium text-gray-500">Direction</label>
                                    <select name="direction" class="mt-1 block w-full rounded-md border border-gray-300 px-3 py-2 text-sm shadow-sm focus:border-[#0092b4] focus:outline-none focus:ring-1 focus:ring-[#0092b4]">
                                        <option value="Inbound" @selected($comm->direction === 'Inbound')>Inbound</option>
                                        <option value="Outbound" @selected($comm->direction === 'Outbound')>Outbound</option>
                                    </select>
                                </div>
                                <div class="sm:col-span-2">
                                    <label class="block text-xs font-medium text-gray-500">Subject</label>
                                    <input type="text" name="subject" value="{{ $comm->subject }}" class="mt-1 block w-full rounded-md border border-gray-300 px-3 py-2 text-sm shadow-sm focus:border-[#0092b4] focus:outline-none focus:ring-1 focus:ring-[#0092b4]">
                                </div>
                                <div class="sm:col-span-2">
                                    <label class="block text-xs font-medium text-gray-500">Summary</label>
                                    <textarea name="summary" rows="2" class="mt-1 block w-full rounded-md border border-gray-300 px-3 py-2 text-sm shadow-sm focus:border-[#0092b4] focus:outline-none focus:ring-1 focus:ring-[#0092b4]">{{ $comm->summary }}</textarea>
                                </div>
                                <div>
                                    <label class="block text-xs font-medium text-gray-500">Date/Time</label>
                                    <input type="datetime-local" name="communication_date" value="{{ $comm->communication_date ? \Carbon\Carbon::parse($comm->communication_date)->format('Y-m-d\TH:i') : '' }}" class="mt-1 block w-full rounded-md border border-gray-300 px-3 py-2 text-sm shadow-sm focus:border-[#0092b4] focus:outline-none focus:ring-1 focus:ring-[#0092b4]">
                                </div>
                                <div class="flex items-end justify-end gap-2">
                                    <button type="button" @click="editing = false" class="rounded-lg border border-gray-300 px-3 py-2 text-xs text-gray-700 hover:bg-gray-100">Cancel</button>
                                    <button type="submit" class="rounded-lg bg-[#0092b4] px-3 py-2 text-xs text-white hover:bg-[#007a9a]">Save</button>
                                </div>
                            </form>
                        </div>
            ...
            <div class="rounded-lg border border-gray-200 bg-white p-6">
                ...
                <form id="addEnqDocForm" method="POST" action="{{ route('documents.store') }}" enctype="multipart/form-data" class="hidden mb-4 grid gap-3 rounded-md border border-gray-200 bg-gray-50 p-4">
                    ...
                    @endif
                    <div>
                        <label class="block text-xs font-medium text-gray-500">Document Title</label>
                        <input type="text" name="title" placeholder="e.g. GP Referral Letter, Medical Report…" class="mt-1 block w-full rounded-md border border-gray-300 px-3 py-2 text-sm shadow-sm focus:border-[#0092b4] focus:outline-none focus:ring-1 focus:ring-[#0092b4]">
                    </div>
                    <div>
